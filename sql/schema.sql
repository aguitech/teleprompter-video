...
CREATE TABLE IF NOT EXISTS `sesiones` (
  ...
  `brand` VARCHAR(32) NOT NULL DEFAULT 'bancoppel',
  `chroma_color` VARCHAR(7) DEFAULT '#00ff00',
  `tolerance` INT DEFAULT 80,
  `bg_mode` VARCHAR(32) DEFAULT 'transparent',
  `bg_color` VARCHAR(7) DEFAULT '#003D7A',
  `scenes_count` INT DEFAULT 0,
  `duration` DECIMAL(10,2) DEFAULT 0,
  `output_final` VARCHAR(500) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `escenas` (
  ...
  `chroma_hits` INT DEFAULT 0,
  `bg_replaced` INT DEFAULT 0,
